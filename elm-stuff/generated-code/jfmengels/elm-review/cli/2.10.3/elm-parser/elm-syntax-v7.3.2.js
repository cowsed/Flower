...
var $stil4m$elm_syntax$Elm$Syntax$Expression$functionRange = function (_function) {
	var _v0 = $stil4m$elm_syntax$Elm$Syntax$Node$value(_function.al);
	var name = _v0.L;
	var expression = _v0.C;
	var startRange = function () {
		...
		if (!_v1.$) {
			...
			var _v2 = _function.av;
			if (!_v2.$) {
				var _v3 = _v2.a;
				var value = _v3.b;
				return $stil4m$elm_syntax$Elm$Syntax$Node$range(value.L);
			} else {
	...
	return {
		x: $stil4m$elm_syntax$Elm$Syntax$Node$range(expression).x,
		z: startRange.z
	};
};
		...
		function (_v0) {
			...
			return A2(
				...
				consumeArgs ? A3(
					...
					_Utils_Tuple2(range.x, _List_Nil)));
		},
		A2(
			$stil4m$elm_syntax$Combine$ignore,
			$stil4m$elm_syntax$Combine$maybe($stil4m$elm_syntax$Elm$Parser$Layout$layout),
				...
				function (f) {
					return $stil4m$elm_syntax$Elm$Parser$Declarations$liftRecordAccess(
						A2(
							$stil4m$elm_syntax$Elm$Syntax$Node$Node,
							{
								x: $stil4m$elm_syntax$Elm$Syntax$Node$range(f).x,
								z: $stil4m$elm_syntax$Elm$Syntax$Node$range(e).z
							},
								...
								function (args, expr) {
									return A2(
										$stil4m$elm_syntax$Elm$Syntax$Node$Node,
										{
											...
											z: $stil4m$elm_syntax$Elm$Syntax$Node$range(varPointer).z
										},
					...
					$stil4m$elm_syntax$Combine$succeed(
						function (expr) {
							return A2(
								...
								{
									x: $stil4m$elm_syntax$Elm$Syntax$Node$range(expr).x,
									z: $stil4m$elm_syntax$Elm$Syntax$Node$range(pattern).z
								},
								...
								F3(
									function (caseKeyword, caseBlock_, _v11) {
										...
										return A2(
											...
											{
												x: end,
												z: $stil4m$elm_syntax$Elm$Syntax$Node$range(caseKeyword).z
											},
											$stil4m$elm_syntax$Elm$Syntax$Expression$CaseExpression(
		...
		function (s) {
			return $stil4m$elm_syntax$Combine$withLocation(
				function (l) {
					return _Utils_eq(
						$stil4m$elm_syntax$Elm$Parser$State$expectedColumn(s),
						l.s) ? $stil4m$elm_syntax$Elm$Parser$Declarations$cyclic$caseStatement() : $stil4m$elm_syntax$Combine$fail('Indentation is incorrect to be a case statement');
				});
		});
}
						...
						if (rest.a.b.$ === 6) {
							...
							return $stil4m$elm_syntax$Combine$succeed(
								A2(
									$stil4m$elm_syntax$Elm$Syntax$Node$Node,
									{
										x: lastExpressionRange.x,
										z: $stil4m$elm_syntax$Elm$Syntax$Node$range(first).z
									},
									$stil4m$elm_syntax$Elm$Syntax$Expression$Application(
										A2(
											$elm$core$List$cons,
				...
				F2(
					function (_v0, sig) {
						...
						return A2(
							$stil4m$elm_syntax$Elm$Syntax$Node$Node,
							{
								x: $stil4m$elm_syntax$Elm$Syntax$Node$range(sig.ad).x,
								z: start
							},
														...
														F3(
															function (name, generics, typeAnnotation) {
																return A2(
																	$stil4m$elm_syntax$Elm$Syntax$Node$Node,
																	{
																		x: $stil4m$elm_syntax$Elm$Syntax$Node$range(typeAnnotation).x,
																		z: start
																	},
																	$stil4m$elm_syntax$Elm$Syntax$Declaration$AliasDeclaration(
																		{I: $elm$core$Maybe$Nothing, T: generics, L: name, ad: typeAnnotation}));
															}))))))))))),
					A2(
					$stil4m$elm_syntax$Combine$keep,
												...
												F3(
													function (name, generics, constructors) {
														...
														return A2(
															...
															$stil4m$elm_syntax$Elm$Syntax$Declaration$CustomTypeDeclaration(
																{
																	aj: $elm$core$List$reverse(constructors),
																	I: $elm$core$Maybe$Nothing,
																	T: generics,
																	L: name
																}));
		...
		var endRange = function () {
			...
			if (!_v0.$) {
				...
				if (!_v1.$) {
					...
					return $stil4m$elm_syntax$Elm$Syntax$Node$range(imp.F);
				}
			}
		}();
		return A2(
			$stil4m$elm_syntax$Elm$Syntax$Node$Node,
			{x: endRange.x, z: start},
			imp);
	});
var $stil4m$elm_syntax$Elm$Parser$Imports$importDefinition = function () {
	...
	var parseAsDefinition = F2(function parseAsDefinition$fn(_v0, mod) {
			...
			return A2(
				$stil4m$elm_syntax$Combine$map,
				$stil4m$elm_syntax$Elm$Parser$Imports$setupNode(importKeywordRange.z),
				$stil4m$elm_syntax$Combine$oneOf(
					_List_fromArray(
						[
							A2(
			...
			return {
				Q: A2(
					...
					A2(
						$stil4m$elm_syntax$Elm$Syntax$Node$Node,
						{
							x: $stil4m$elm_syntax$Elm$Syntax$Node$range(declaration).x,
							z: $stil4m$elm_syntax$Elm$Syntax$Node$range(doc).z
						},
						howToUpdate(doc)),
					file.Q),
...
var $stil4m$elm_syntax$Elm$Syntax$Range$combineHelp = F3(function $stil4m$elm_syntax$Elm$Syntax$Range$combineHelp$fn(ranges, previousStart, previousEnd) {
		...
		while (true) {
			if (!ranges.b) {
				return {x: previousEnd, z: previousStart};
			} else {
				var start = ranges.a.z;
				var end = ranges.a.x;
				var rest = ranges.b;
...
var $stil4m$elm_syntax$Elm$Syntax$Range$combine = function (ranges) {
	if (!ranges.b) {
		...
		var head = ranges.a;
		var tail = ranges.b;
		return A3($stil4m$elm_syntax$Elm$Syntax$Range$combineHelp, tail, head.z, head.x);
	}
};
			...
			switch (expression.$) {
				...
				case 15:
					...
					return $stil4m$elm_syntax$Elm$Syntax$Expression$LetExpression(
						{
							Q: $stil4m$elm_syntax$Elm$Processing$visitLetDeclarations(letBlock.Q),
							C: $stil4m$elm_syntax$Elm$Processing$visitExpression(letBlock.C)
						});
				case 16:
					...
					return $stil4m$elm_syntax$Elm$Syntax$Expression$CaseExpression(
						{
							ag: A2(
								...
								caseBlock.ag),
							C: $stil4m$elm_syntax$Elm$Processing$visitExpression(caseBlock.C)
						});
				case 17:
					...
					return $stil4m$elm_syntax$Elm$Syntax$Expression$LambdaExpression(
						_Utils_update(
							lambda,
							{
								C: $stil4m$elm_syntax$Elm$Processing$visitExpression(lambda.C)
							}));
				case 18:
...
var $stil4m$elm_syntax$Elm$Processing$visitFunctionDecl = function (_function) {
	var newFunctionDeclaration = A2($stil4m$elm_syntax$Elm$Syntax$Node$map, $stil4m$elm_syntax$Elm$Processing$visitFunctionDeclaration, _function.al);
	return _Utils_update(
		_function,
		{al: newFunctionDeclaration});
};
var $stil4m$elm_syntax$Elm$Processing$visitFunctionDeclaration = function (functionDeclaration) {
	var newExpression = $stil4m$elm_syntax$Elm$Processing$visitExpression(functionDeclaration.C);
	return _Utils_update(
		functionDeclaration,
		{C: newExpression});
};
...
var $stil4m$elm_syntax$Elm$Syntax$Range$compare = F2(function $stil4m$elm_syntax$Elm$Syntax$Range$compare$fn(left, right) {
		var _v0 = A2($stil4m$elm_syntax$Elm$Syntax$Range$compareLocations, left.z, right.z);
		if (_v0 === 1) {
			return A2($stil4m$elm_syntax$Elm$Syntax$Range$compareLocations, left.x, right.x);
		} else {
			var order = _v0;
			return order;
		}
